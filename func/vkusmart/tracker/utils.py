...
def bbox_belongs_to_zone(bb: list, zones: list, eps = 0):
    
    def belong(zone):
        return bb[0] > zone[0]-eps and bb[1] > zone[1]-eps and bb[2] < zone[2]+eps and bb[3] < zone[3]+eps
    
    if type(zones[0]) == tuple:
        for zone_info in zones:
            if belong(zone_info[1]):
                return True
    else:
        if belong(zones):
            return True
...
def bbox_interseq_with_zone(bb: list, zones: list, eps = 0):
    def interseq(zone): # shit, тут подвох, но "работает-- не трогай" (до релиза)
        if bb[2] < zone[0] or bb[0] > zone[2]: # чисто слева или чисто справа по x
            return False
        if bb[3] < zone[1] or bb[1] > zone[3]: # чисто вышие или чисто ниже по y
            return False
        return True
    if type(zones[0]) == tuple:
        for zone_info in zones:
            zone = zone_info[1]
            if interseq(zone_info[1]):
                return True
    elif type(zones[0]) == list:
        for zone_info in zones:
            if interseq(zone_info):
                return True
    else: 
        if interseq(zones): 
            return True
...
def is_interseq_with_img_border(bb: list, w: int, h: int):
    def interseq(zone):
        if bb[2] < zone[0] or bb[0] > zone[2]: # чисто слева или чисто справа по x
            return False
        if bb[3] < zone[1] or bb[1] > zone[3]: # чисто вышие или чисто ниже по y
            return False
        return True
    if interseq([-1, -1, w, 0]) or interseq([-1,-1,0,h]) or interseq([-1,h,w,h+1]) or interseq([w,-1,w+1,h]):
        return True
    return False
